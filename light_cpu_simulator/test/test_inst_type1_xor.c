...
#undef  TESTNAME
#define TESTNAME inst_type1_xor
...
SET_TEST(TESTNAME, test_type1_xor)
{
	...
	SET_TYPE1_XOR(InstMem[0], RD, RB, RA);
...
SET_TEST(TESTNAME, test_type1_not_xor)
{
	int InstSize, i,Value1, Value2;
	int RA, RB, RD;
	...
	SET_TYPE1_NOT_XOR(InstMem[0], RD, RB, RA);
	SET_CPU_REG(RA, Value1);
	SET_CPU_REG(RB, Value2);
...
SET_TEST_GROUP_RUNNER(TESTNAME)
{
	...
	SET_RUN_TEST_CASE(TESTNAME, test_type1_xor);
    SET_RUN_TEST_CASE(TESTNAME, test_type1_not_xor);
